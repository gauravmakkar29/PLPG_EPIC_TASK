...
async function main(): Promise<void> {
  ...
  const pythonBasics = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'Python is the lingua franca of machine learning. Mastering NumPy and Pandas will enable you to efficiently manipulate datasets and prepare data for ML models, skills you will use daily as an ML engineer.',
      phase: 'foundation',
      estimatedHours: 8,
      isOptional: false,
      sequenceOrder: 1,
  ...
  const mathFoundations = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'Understanding the math behind ML algorithms allows you to debug models, optimize performance, and innovate beyond out-of-the-box solutions. This foundation separates practitioners from true engineers.',
      phase: 'foundation',
      estimatedHours: 12,
      isOptional: false,
      sequenceOrder: 2,
  ...
  const dataPreprocessing = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'Real-world data is messy. Up to 80% of an ML engineer\'s time is spent on data preprocessing. These skills directly impact model accuracy and are essential for production-ready systems.',
      phase: 'foundation',
      estimatedHours: 6,
      isOptional: false,
      sequenceOrder: 3,
  ...
  const mlAlgorithms = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'These classical algorithms form the backbone of many production ML systems. Understanding when to use each algorithm and their trade-offs is crucial for solving real business problems efficiently.',
      phase: 'intermediate',
      estimatedHours: 15,
      isOptional: false,
      sequenceOrder: 4,
  ...
  const deepLearning = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'Deep learning powers cutting-edge applications from ChatGPT to self-driving cars. This skill opens doors to the most exciting and high-impact ML applications in the industry.',
      phase: 'intermediate',
      estimatedHours: 20,
      isOptional: false,
      sequenceOrder: 5,
  ...
  const modelDeployment = await prisma.skill.create({
    data: {
      ...
      whyThisMatters: 'A model that stays in a notebook creates no value. Your backend experience gives you an advantage here—combining it with MLOps skills makes you uniquely valuable for getting models into production.',
      phase: 'advanced',
      estimatedHours: 10,
      isOptional: false,
      sequenceOrder: 6,
